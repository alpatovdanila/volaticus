// clear-synth-sfx.ts — delete every SYNTHESIZED sfx (the `layers` form) and remove
// all references to it across the inventory. Mirror of clear-imported-sfx.ts (which
// handled the sample `files` form); together they leave inventory/sfx empty.
//
// A synth sfx has a `layers` array (WebAudio patch); it has no `files`. References
// live only in entity bindings (states.*.enter, states.*.cues, states.*.ambient,
// events.*) and in effect docs (`sfx`: string | string[]). Removing a ref that
// leaves an empty binding / cue / event / byContext override prunes it; an
// `ambient` block (schema REQUIRES its sfx) is removed whole. Named states are
// kept even if emptied. Co-located non-sfx fields (anim/effect/flash/...) are kept.
//
...
//
//   npx tsx scripts/clear-synth-sfx.ts [--dry]
...
// 1. collect the synthesized (layers-based) sfx ids.
...
for (const f of fs.readdirSync(path.join(INV, 'sfx')).filter((f) => f.endsWith('.json'))) {
  ...
  if (Array.isArray(doc.layers)) removed.add(doc.id ?? f.replace(/\.json$/, ''))
}
console.log(`synth sfx to clear (${removed.size}): ${[...removed].sort().join(', ')}`)
...
console.log(`remaining sfx files: ${fs.readdirSync(path.join(INV, 'sfx')).filter((f) => f.endsWith('.json')).length}`)
